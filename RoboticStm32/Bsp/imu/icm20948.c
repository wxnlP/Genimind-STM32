...
void ICM20948_RealData(ImuReal* accReal, ImuReal* gyroReal) {
    /* 加速度真实值: 寄存器值/4096 ; 4096 = 65535/16 = 16位ADC/量程; 单位：g
     * 角速度真实值: 寄存器值/16.4 ; 16.4 = 65535/4000 = 16位ADC/量程; 单位：°/s
     * */
    ImuAccel accData;
    ImuGyro gyroData;
    /* 获取模拟值 */
...
/* 读取数据 */
uint8_t AK09916_ReadMag(ImuMag* magData) {
    if (!AK09916_ReadByte(AK09916_STATUS_1)) {
        return 0;
    }
    ...
    if ((AK09916_ReadByte(AK09916_STATUS_2) & 0x08)) {
        return 0;
    }
...
/* 获取磁力计的真实值 */
uint8_t AK09916_RealData(ImuReal* magReal) {
    /* 磁力计灵敏度为0.15 µT/LSB，ADC模拟值*0.15 */
    ImuMag magData;
    if (!AK09916_ReadMag(&magData)) {
        return 0;
    }
    ...
    float norm = sqrtf(magReal->X*magReal->X + magReal->Y*magReal->Y + magReal->Z*magReal->Z);
    if (norm < 20.0f || norm > 60.0f) { // 地磁场正常范围20~60μT
        return 0; // 数据异常，返回0
    }
    return 1;
}
